...
class EmbedFactory:
    ...
    def create_mission_embed(
        ...
        **kwargs
        ...
        """Create a specialized mission embed with complete normalization support"""
        ...
        fields = []
        if mission_id:
            fields.append({"name": "Mission ID", "value": f"`{mission_id}`", "inline": True})
        if level:
            stars = "⭐" * min(level, 5)
            fields.append({"name": "Difficulty", "value": f"{stars} Tier {level}", "inline": True})
        if state:
            icon = state_icons.get(state, '📋')
            fields.append({"name": "Status", "value": f"{icon} {state}", "inline": True})
        if respawn_time:
            fields.append({"name": "Respawn Time", "value": f"{respawn_time}s", "inline": True})
        if location:
        ...
        embed.set_footer(text="Emerald's Killfeed • Mission System", 
    ...
    def create_connection_embed(
        ...
        """Create a player connection event embed"""
        embed = discord.Embed(
            title=title,
            description=description,
            color=color or cls.COLORS['player_join'],
            timestamp=timestamp or datetime.now(ZoneInfo('UTC'))
        )
        ...
        embed.set_footer(text="Emerald's Killfeed • Connection System", 
                        icon_url="attachment://main.png")
        return embed
    ...
    def _build_suicide(cls, data: Dict[str, Any]) -> tuple[discord.Embed, Optional[discord.File]]:
        """Build suicide embed with simplified dark humor format"""
        title = random.choice(cls.TITLE_POOLS['suicide'])

        # Dark humor suicide messages
        suicide_messages = [
            "One button press later and {player_name} is no more.",
            "{player_name} decided to press the wrong button.",
            "{player_name} chose the express checkout option.",
            "{player_name} found the ultimate rage quit method.",
            "{player_name} accidentally hit the self-destruct button.",
            "{player_name} took the easy way out of this nightmare.",
            "{player_name} couldn't handle the pressure anymore.",
            "{player_name} found their own personal exit door."
        ]

        player_name = data.get('player_name', 'Unknown')
        suicide_message = random.choice(suicide_messages).format(player_name=player_name)
        combat_log = random.choice(cls.COMBAT_LOGS['suicide'])

        # Simple description format
        description = f"{suicide_message}\n\n*{combat_log}*"
        ...
        # Thumbnail
        thumbnail_url = data.get('thumbnail_url', 'attachment://Suicide.png')
        embed.set_thumbnail(url=thumbnail_url)
    ...
    def _build_fall(cls, data: Dict[str, Any]) -> tuple[discord.Embed, Optional[discord.File]]:
        """Build fall damage embed with simplified dark humor format"""
        title = random.choice(cls.TITLE_POOLS['fall'])

        # Dark humor falling messages
        falling_messages = [
            "Gravity got the best of {player_name}.",
            "{player_name} forgot they weren't a bird.",
            "{player_name} took the scenic route down.",
            "{player_name} discovered the hard way that humans can't fly.",
            "{player_name} became one with the pavement.",
            "{player_name} learned about terminal velocity the hard way.",
            "{player_name} went splat faster than expected.",
            "{player_name} made an unscheduled landing."
        ]

        player_name = data.get('player_name', 'Unknown')
        falling_message = random.choice(falling_messages).format(player_name=player_name)
        combat_log = random.choice(cls.COMBAT_LOGS['fall'])

        # Simple description format
        description = f"{falling_message}\n\n*{combat_log}*"
        ...
        # Thumbnail
        thumbnail_url = data.get('thumbnail_url', 'attachment://Falling.png')
        embed.set_thumbnail(url=thumbnail_url)
